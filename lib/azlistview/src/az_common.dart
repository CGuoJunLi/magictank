...
class SuspensionUtil {
  ...
  static List<ISuspensionBean> result = [];
  static void sortListBySuspensionTag(List<ISuspensionBean>? list,
    ...
    list.sort((a, b) {
      if (a.getSuspensionTag() == "@" || b.getSuspensionTag() == "#") {
        ...
        if (model == 1) {
          if (a.getSuspensionTag().compareTo(b.getSuspensionTag()) == 0) {
            if (json.decode(a.toString())["name"].length >
                json.decode(b.toString())["name"].length) {
              return 1;
            } else {
              if (json.decode(a.toString())["name"].length ==
                  json.decode(b.toString())["name"].length) {
